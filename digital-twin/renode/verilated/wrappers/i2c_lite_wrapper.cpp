...
// I2C Controller (lightweight, 0 interrupts) Co-simulation wrapper for Renode
// OpenDrain bus lines: scl_read=1, sda_read=1 (pulled-up idle bus).
// No io_i2c_interrupts port (lightweight uses 0 external interrupts).
...
#if DEBUG_ENABLE
  #define DEBUG_PRINT(fmt, ...) do { printf("[I2C_LITE] " fmt "\n", ##__VA_ARGS__); fflush(stdout); } while(0)
#else
  #define DEBUG_PRINT(fmt, ...)
#endif

static VWishboneI2cController* g_top = nullptr;
...
static void copyBridgeAndEval() {
  ...
  // Tie I2C bus lines high (idle/pulled-up)
  g_top->io_i2c_scl_read = 1;
  g_top->io_i2c_sda_read = 1;
  // NOTE: No io_i2c_interrupts port in lightweight variant
...
class I2cLitePeripheral : public RenodeAgent {
public:
  I2cLitePeripheral() : RenodeAgent(), top(nullptr), bus(nullptr), tickCounter(0) {}

  void initialize() {
    DEBUG_PRINT("=== INITIALIZATION START ===");

    top = new VWishboneI2cController();
    g_top = top;
    ...
    // Tie I2C lines high before reset
  ...
  void tick(bool countEnable, uint64_t steps) override {
    ...
    for (uint64_t i = 0; i < steps; i++) {
      // Maintain I2C bus tie-offs during ticking
      top->io_i2c_scl_read = 1;
      top->io_i2c_sda_read = 1;
...
private:
  VWishboneI2cController* top;
  Wishbone* bus;
  uint64_t tickCounter;
};

static I2cLitePeripheral* i2cLitePeripheral = nullptr;

RenodeAgent* Init() {
  if (!i2cLitePeripheral) {
    i2cLitePeripheral = new I2cLitePeripheral();
    i2cLitePeripheral->connectNative();
    i2cLitePeripheral->initialize();
  }
  return i2cLitePeripheral;
}
