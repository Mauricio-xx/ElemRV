...
// UART Co-simulation wrapper for Renode
...
// Pin tie-offs: rxd=1 (idle mark), cts=0 (asserted/clear-to-send).
...
#if DEBUG_ENABLE
  #define DEBUG_PRINT(fmt, ...) do { printf("[UART] " fmt "\n", ##__VA_ARGS__); fflush(stdout); } while(0)
#else
...
static void copyBridgeAndEval() {
  ...
  // Tie UART pins: RXD high (idle mark), CTS low (asserted)
  g_top->io_uart_rxd = 1;
  g_top->io_uart_cts = 0;
...
class UartPeripheral : public RenodeAgent {
public:
  UartPeripheral() : RenodeAgent(), top(nullptr), bus(nullptr), tickCounter(0) {}
  ...
  void tick(bool countEnable, uint64_t steps) override {
    ...
    for (uint64_t i = 0; i < steps; i++) {
      // Maintain UART pin tie-offs during ticking
      top->io_uart_rxd = 1;
      top->io_uart_cts = 0;
...
static UartPeripheral* uartPeripheral = nullptr;

RenodeAgent* Init() {
  if (!uartPeripheral) {
    uartPeripheral = new UartPeripheral();
    uartPeripheral->connectNative();
    uartPeripheral->initialize();
  }
  return uartPeripheral;
}
