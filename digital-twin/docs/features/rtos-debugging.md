...
This guide covers Zephyr RTOS debugging within the ElemRV Digital Twin, from GDB thread inspection to on-target diagnostics.

## Overview

The digital twin provides multiple levels of RTOS debugging:

1. **Thread-Aware GDB**: Python script for inspecting Zephyr threads
2. **On-Target Diagnostics**: Thread analyzer and logging via UART
3. **Execution Profiling**: Instruction-level tracing
...
riscv-none-elf-gdb -x gdb/elemrv.gdb \
    digital-twin/zephyr/elemrv-zephyr/build-rtos-debug-demo/zephyr/zephyr.elf
```
...
# Terminal 2
riscv-none-elf-gdb -x gdb/elemrv.gdb \
    digital-twin/zephyr/elemrv-zephyr/build-n-rtos-debug-demo/zephyr/zephyr.elf
```
...
Renode 1.16.0's GDB server does not support native RTOS thread awareness (tracked in Renode issue #637). The standard `info threads` command only shows CPU cores, not Zephyr threads.

The `digital-twin/digital-twin/renode/gdb/zephyr_threads.py` script reads Zephyr's kernel data structures directly from memory using GDB's Python API.

### How It Works

When `CONFIG_DEBUG_THREAD_INFO=y` is enabled, Zephyr exports a `_kernel_openocd_offsets` array containing field offsets for thread structures.
...
- Thread inspection is a snapshot at the point GDB interrupts execution. The firmware must be paused (Ctrl-C or breakpoint) before running `zephyr-threads`.
- Stack measurement via sentinel scanning may slightly overcount usage if a thread writes `0xAAAAAAAA` to its stack.
- When Renode eventually adds native `info threads` support (issue #637), the Python script will still work but become redundant.

## Level 2: On-Target Diagnostics via UART

### Thread Analyzer

Zephyr's built-in thread analyzer reports thread names, states, and stack usage via printk/UART.
...
**Auto mode** (H only, needs ~768B stack):
```ini
...
The Zephyr shell provides interactive kernel inspection via UART. Requires ~1.5-2KB RAM - only fits on H (8KB).
...
Renode provides an instruction-level execution profiler that requires zero target RAM. Use it to trace every PC value executed:
...
The profiler output can identify context switches by tracking when the PC jumps between thread entry functions. This gives coarse thread scheduling visibility without any firmware changes.
...
CTF over UART interleaves binary trace data with console output, requiring post-processing to separate. CTF over RAM dump captures a small ring buffer that can be extracted via GDB after stopping execution.
...
SystemView requires SEGGER RTT (Real-Time Transfer), which uses a dedicated debug probe interface. Renode does not emulate RTT, so SystemView is **not viable** in the digital twin.

### Future: Native Thread Awareness

When Renode adds native GDB RTOS thread awareness (issue #637), the standard `info threads` GDB command will show Zephyr threads directly. The `zephyr_threads.py` script will remain functional as a fallback.
...
task dt-debug-connect ELF=digital-twin/zephyr/elemrv-zephyr/build-rtos-debug-demo/zephyr/zephyr.elf
```
...
cd digital-twin/zephyr/elemrv-zephyr
west build -b elemrv_h app/rtos_debug_demo -d build-debug

# 2. Start Renode (Terminal 1)
task dt-rtos-debug

# 3. Connect GDB (Terminal 2)
task dt-debug-connect ELF=digital-twin/zephyr/elemrv-zephyr/build-debug/zephyr/zephyr.elf
...
```
# Check stack usage in thread analyzer
uart:~$ kernel stacks

# Look for high usage:
threadA: threadA 0x80001234 unused 16 usage 240/256 (93%)
                                     ^^^^^^^^^^^^^^^^
...
# In GDB:
(gdb) zephyr-threads
# Look for threads stuck in PENDING or BLOCKED state

# Example:
# thread_a  PENDING    5     ...   # Waiting for mutex?
# thread_b  PENDING    6     ...   # Also waiting?
# -> Potential deadlock
```

---

**Previous**: [Sensor Simulation](sensors.md)  
**Next**: [GDB Debugging](gdb-debugging.md)
