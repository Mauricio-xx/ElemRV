...
task dt-debug-connect ELF=digital-twin/firmware/pwm_test/pwm_test.elf
```
...
task dt-debug-connect ELF=digital-twin/zephyr/elemrv-zephyr/build/zephyr/zephyr.elf
```
...
The `digital-twin/digital-twin/renode/gdb/elemrv.gdb` script is loaded automatically and provides peripheral register inspection commands.
...
# Terminal 2
docker exec -it elemrv-test bash -c \
  'cd /workspace/elemrv && riscv-none-elf-gdb -x digital-twin/renode/gdb/elemrv.gdb digital-twin/firmware/pwm_test/pwm_test.elf'
```
...
riscv-none-elf-gdb -x digital-twin/renode/gdb/elemrv.gdb digital-twin/firmware/pwm_test/pwm_test.elf
```
...
renode --disable-xwt --console \
  -e '$elf = @../digital-twin/firmware/pwm_test/pwm_regtest.elf' \
  -e 'include @debug_bare_metal.resc'
```

Or set environment variable:
```bash
export ELF=digital-twin/firmware/pwm_test/pwm_test.elf
task dt-debug-connect
```
...
(gdb) file digital-twin/firmware/pwm_test/pwm_test.elf

# Or specify at connection
$ riscv-none-elf-gdb -x digital-twin/renode/gdb/elemrv.gdb firmware.elf
```
