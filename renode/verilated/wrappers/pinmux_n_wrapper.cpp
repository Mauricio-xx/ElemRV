...
// Pinmux (N-variant, 20 pins) Co-simulation wrapper for Renode
// Register-only peripheral: 1 register per pin at pin*4 offset.
// No IpIdentification header. TriState I/O pins left unconnected.
...
#if DEBUG_ENABLE
  #define DEBUG_PRINT(fmt, ...) do { printf("[PINMUX_N] " fmt "\n", ##__VA_ARGS__); fflush(stdout); } while(0)
#else
  #define DEBUG_PRINT(fmt, ...)
#endif

static VWishbonePinmux* g_top = nullptr;
...
class PinmuxNPeripheral : public RenodeAgent {
public:
  PinmuxNPeripheral() : RenodeAgent(), top(nullptr), bus(nullptr), tickCounter(0) {}

  void initialize() {
    DEBUG_PRINT("=== INITIALIZATION START ===");

    top = new VWishbonePinmux();
    g_top = top;
...
static PinmuxNPeripheral* pinmuxNPeripheral = nullptr;

RenodeAgent* Init() {
  if (!pinmuxNPeripheral) {
    pinmuxNPeripheral = new PinmuxNPeripheral();
    pinmuxNPeripheral->connectNative();
    pinmuxNPeripheral->initialize();
  }
  return pinmuxNPeripheral;
}
