...
// Pinmux Co-simulation wrapper for Renode
...
#if DEBUG_ENABLE
  #define DEBUG_PRINT(fmt, ...) do { printf("[PINMUX] " fmt "\n", ##__VA_ARGS__); fflush(stdout); } while(0)
#else
...
class PinmuxPeripheral : public RenodeAgent {
public:
  PinmuxPeripheral() : RenodeAgent(), top(nullptr), bus(nullptr), tickCounter(0) {}
...
static PinmuxPeripheral* pinmuxPeripheral = nullptr;

RenodeAgent* Init() {
  if (!pinmuxPeripheral) {
    pinmuxPeripheral = new PinmuxPeripheral();
    pinmuxPeripheral->connectNative();
    pinmuxPeripheral->initialize();
  }
  return pinmuxPeripheral;
}
