...
// I2C Controller Co-simulation wrapper for Renode
// OpenDrain bus lines: scl_read=1, sda_read=1 (pulled-up idle bus).
// io_i2c_interrupts[0]: external interrupt input, tied to 0.
...
#if DEBUG_ENABLE
  #define DEBUG_PRINT(fmt, ...) do { printf("[I2C] " fmt "\n", ##__VA_ARGS__); fflush(stdout); } while(0)
#else
...
class I2cPeripheral : public RenodeAgent {
public:
  I2cPeripheral() : RenodeAgent(), top(nullptr), bus(nullptr), tickCounter(0) {}
...
static I2cPeripheral* i2cPeripheral = nullptr;

RenodeAgent* Init() {
  if (!i2cPeripheral) {
    i2cPeripheral = new I2cPeripheral();
    i2cPeripheral->connectNative();
    i2cPeripheral->initialize();
  }
