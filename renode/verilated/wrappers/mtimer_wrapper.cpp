...
// MachineTimer Co-simulation wrapper for Renode
// RISC-V standard mtime/mtimecmp layout (NOT LiteX timer).
// io_interrupt output: ignored (not connected to CPU in co-sim test).
// No IpIdentification header.
...
#if DEBUG_ENABLE
  #define DEBUG_PRINT(fmt, ...) do { printf("[MTIMER] " fmt "\n", ##__VA_ARGS__); fflush(stdout); } while(0)
#else
  #define DEBUG_PRINT(fmt, ...)
#endif

static VWishboneMachineTimer* g_top = nullptr;
...
class MTimerPeripheral : public RenodeAgent {
public:
  MTimerPeripheral() : RenodeAgent(), top(nullptr), bus(nullptr), tickCounter(0) {}

  void initialize() {
    DEBUG_PRINT("=== INITIALIZATION START ===");

    top = new VWishboneMachineTimer();
    g_top = top;
...
private:
  VWishboneMachineTimer* top;
  Wishbone* bus;
  uint64_t tickCounter;
};

static MTimerPeripheral* mtimerPeripheral = nullptr;

RenodeAgent* Init() {
  if (!mtimerPeripheral) {
    mtimerPeripheral = new MTimerPeripheral();
    mtimerPeripheral->connectNative();
    mtimerPeripheral->initialize();
  }
  return mtimerPeripheral;
}
