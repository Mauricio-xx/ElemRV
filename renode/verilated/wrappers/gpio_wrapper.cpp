...
// GPIO Co-simulation wrapper for Renode
...
#if DEBUG_ENABLE
  #define DEBUG_PRINT(fmt, ...) do { printf("[GPIO] " fmt "\n", ##__VA_ARGS__); fflush(stdout); } while(0)
#else
...
class GpioPeripheral : public RenodeAgent {
public:
  GpioPeripheral() : RenodeAgent(), top(nullptr), bus(nullptr), tickCounter(0) {}
...
static GpioPeripheral* gpioPeripheral = nullptr;

RenodeAgent* Init() {
  if (!gpioPeripheral) {
    gpioPeripheral = new GpioPeripheral();
    gpioPeripheral->connectNative();
