...
**reuse lint.** 246 tracked files missing copyright/license info. Biggest bucket: 116 files in `software/elemrv-zephyr/` (out-of-tree Zephyr module) — most declared Apache-2.0 inline but lacked `SPDX-FileCopyrightText`. Other buckets: DT docs, test driver `.resc` files, platform `.repl` files, bare-metal firmware build artefacts (`.bin/.elf/.lst/.img`), generated Verilog under `gen/**` + `gen_n/**`, tracked `.gitignore` files, per-user `.claude/settings.local.json` (gitignored but reuse 6.2.0 does not honour `.gitignore`).
...
- `hardware/scala/elemrv_h/test/SimpleWishboneReg.scala`
- `hardware/scala/elemrv_h/test/WishbonePwmVerilog.scala`
- `hardware/scala/elemrv_n/test/WishboneBmbBridgeVerilog.scala`
- `hardware/scala/elemrv_n/test/WishboneBmbSpiXipControllerVerilog.scala`
